(function(){
  ...
  function blocoHTML(c){
    return `
      <div class="tu27-summary-title">ANUIDADES E PLANOS 2027</div>
      <div class="tu27-annual-grid">
        <div class="tu27-annual due"><small>Anuidade 2027 — até vencimento</small><b data-tu27="annDue">${money(c.annDue)}</b></div>
        <div class="tu27-annual late"><small>Anuidade 2027 — após vencimento</small><b data-tu27="annLate">${money(c.annLate)}</b></div>
      </div>
      <div class="tu27-plan-grid">
        <div class="tu27-plan"><span>PLANO A</span><small>Até o vencimento</small><b data-tu27="aDue">12 × ${money(c.aDue)}</b><em>1ª parcela ${money(c.first)} + 12 parcelas</em><strong>Total: <i data-tu27="totalADue">${money(c.annDue)}</i></strong></div>
        <div class="tu27-plan late"><span>PLANO A</span><small>Após o vencimento</small><b data-tu27="aLate">12 × ${money(c.aLate)}</b><em>1ª parcela ${money(c.first)} + 12 parcelas</em><strong>Total: <i data-tu27="totalALate">${money(c.annLate)}</i></strong></div>
        <div class="tu27-plan"><span>PLANO B</span><small>Até o vencimento</small><b data-tu27="bDue">11 × ${money(c.bDue)}</b><em>1ª parcela ${money(c.first)} + 11 parcelas</em><strong>Total: <i data-tu27="totalBDue">${money(c.annDue)}</i></strong></div>
        <div class="tu27-plan late"><span>PLANO B</span><small>Após o vencimento</small><b data-tu27="bLate">11 × ${money(c.bLate)}</b><em>1ª parcela ${money(c.first)} + 11 parcelas</em><strong>Total: <i data-tu27="totalBLate">${money(c.annLate)}</i></strong></div>
      </div>`;
  }
  ...
  function bindCard(card){
    if(card.dataset.tu27Bound==='1'){update(card);return;}
    card.dataset.tu27Bound='1';
    ['.tu-rate','.tu-annuity','.tu-first'].forEach(sel=>{
      const el=card.querySelector(sel);
      if(el)el.addEventListener('input',()=>setTimeout(()=>update(card),0));
      if(el)el.addEventListener('change',()=>setTimeout(()=>update(card),0));
    });
    update(card);
  }
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='tu27-style';s.textContent=`
      ...
      .tu27-annual b{font-size:22px;color:#168047}
      .tu27-annual.late b{color:#9a6400}
      ...
      .tu27-plan small{font-weight:800;color:#526d88}
      .tu27-plan b{display:block;font-size:20px;color:#15904f;margin:9px 0 4px}
      .tu27-plan.late b{color:#a86b00}
      .tu27-plan em{display:block;font-style:normal;color:#6e8092;font-size:12px;margin-bottom:7px}
      .tu27-plan strong{display:block;color:#0d315c;border-top:1px solid #e2eaf2;padding-top:7px}
      .tu27-plan strong i{font-style:normal;float:right}
