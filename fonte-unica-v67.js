...
function productRows(db){const out=[];Object.entries(db.records||{}).forEach(([storedId,r])=>{if(!r?.saved)return;const category=r.category==='Materiais / Livros'?'Material Didático':(r.category||'Produto 2027');const prefix=category==='Fardamento'?'FAR':category==='STI / Tempo Integral'?'STI':'MATD';const id=/^(FAR|STI|MATD)\|/.test(String(storedId))?storedId:`${prefix}|${slug(storedId)}`;out.push(approved(id,category,r.segment,r.item,r.detail,r.v26,r.v27,r.rate,{obrigatorio:category==='Material Didático'?'SIM':'NÃO',observacao:`Registro ${storedId} salvo em ${r.savedAt||now()}`}))});return out}
...
function init(){ /* v71: publicação automática removida; o servidor é a fonte oficial. */ }
