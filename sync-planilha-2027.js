(function(){
  ...
  function tuitionRows(){
    ...
    Object.entries(db.tuition||{}).forEach(([seg,r])=>{
      ...
      out.push({id:'MEN|'+slug(seg),data:{'Categoria':'Mensalidade','Segmento/Turma':seg,'Produto':'Anuidade + Planos A/B','Descrição':'Anuidade até vencimento e referência após vencimento. 1ª parcela definida pela gestão.','Valor 2026':Number(r.base26)||0,'Valor 2027':ann,'Reajuste %':rate,'Parcelamento':`1ª parcela R$ ${first.toFixed(2)} | Plano A: 12 x R$ ${a.toFixed(2)} | Plano B: 11 x R$ ${b.toFixed(2)}`,'Obrigatório':'SIM','Status':'APROVADO','Aprovado em':now(),'Publicado no Cora Família':'SIM','Publicado em':now(),'ID':'MEN|'+slug(seg),'Observação':`Após vencimento 2026: R$ ${late26.toFixed(2)} | Após vencimento 2027: R$ ${late27.toFixed(2)} | Salvo em ${r.savedAt||now()}`}});
    });
  ...
  function notifyUnified(source,count){document.dispatchEvent(new CustomEvent('cora:official-values',{detail:{source:source||'planilha',count:Number(count)||0,at:new Date().toISOString()}}));}
  function refreshUI(){try{if(typeof window.renderAll==='function')window.renderAll()}catch(e){}try{if(typeof window.CoraReceitaRefresh==='function')window.CoraReceitaRefresh()}catch(e){}notifyUnified('planilha');setTimeout(()=>{try{document.querySelector('#nav button.active')?.click()}catch(e){}},100)}
