(function(){
  ...
  function render(){
    ...
    host.innerHTML=`<div class="mp-head"><div><small>PORTFÓLIO DE MENSALIDADES</small><h2>Mensalidades por segmento</h2><p>Histórico, fechamento oficial 2027, evolução gráfica, condições comerciais e serviços STI relacionados ao segmento selecionado.</p></div><button id="mpOpenClosing">⚙️ Abrir Fechamento 2027</button></div>
      ...
      <div class="mp-card mp-evolution"><div class="mp-section-title"><div><small>EVOLUÇÃO</small><h3>Evolução da anuidade</h3></div><span>2025 → 2029</span></div>${bar([v25,v26,v27,v28,v29],['2025','2026','2027','2028*','2029*'])}<p class="mp-note">* 2028 e 2029 são projeções visuais mantendo a mesma taxa usada em 2027 (${pct(rate)}). Não são valores fechados.</p></div>
      <div class="mp-card"><div class="mp-section-title"><div><small>CONDIÇÕES COMERCIAIS 2027</small><h3>Entrada e parcelamento</h3></div><span>${r.status}</span></div><div class="mp-plans-grid">${planBox('PLANO A','due',first,{qtd:12,valor:aDue},v27)}${planBox('PLANO A','late',first,{qtd:12,valor:aLate},late27)}${planBox('PLANO B','due',first,{qtd:11,valor:bDue},v27)}${planBox('PLANO B','late',first,{qtd:11,valor:bLate},late27)}</div><div class="mp-plan-note"><b>Regra:</b> a entrada/1ª parcela é definida no Fechamento 2027. O saldo restante é dividido em 12 parcelas no Plano A ou 11 parcelas no Plano B.</div></div>
      <div class="mp-grid"><div class="mp-card"><h3>Comparativo — até vencimento</h3><div class="mp-compare"><div><small>2025</small><b>${money(b.due25)}</b></div><div><small>2026</small><b>${money(b.due26)}</b></div><div class="future"><small>2027</small><b>${money(v27)}</b></div></div></div>
      <div class="mp-card"><h3>Comparativo — após vencimento</h3><div class="mp-compare"><div><small>2025</small><b>${money(b.late25)}</b></div><div><small>2026</small><b>${money(b.late26)}</b></div><div class="future"><small>2027</small><b>${money(late27)}</b></div></div></div></div>
      <div class="mp-card mp-sti"><div class="mp-section-title"><div><small>STI / TEMPO INTEGRAL</small><h3>Serviços relacionados ao segmento</h3></div><span>${sti.length?sti.filter(x=>x.status==='OFICIAL').length+'/'+sti.length+' oficiais':'SEM PRODUTO'}</span></div>${sti.length?`<div class="mp-sti-grid">${sti.map(x=>`<div class="mp-sti-item"><div><small>${x.status}</small><h4>${x.name}</h4></div><div class="mp-sti-values"><span>2026 <b>${money(x.v26)}</b></span><span>2027 <b>${money(x.v27)}</b></span><span>Reajuste <b>${pct(x.rate)}</b></span></div></div>`).join('')}</div><div class="mp-plan-note">Os valores 2027 do STI são lidos diretamente do <b>Fechamento 2027</b>. A quantidade é informada na aba <b>Receita</b> para calcular o potencial.</div>`:`<div class="mp-empty">Não há produto STI cadastrado para ${current}.</div>`}</div>
